document.addEventListener("DOMContentLoaded", function () {
  ...
  function initVidTwoSection() {
    ...
    const shoutoutText = document.querySelector('.line3');
    if (shoutoutText) {
      shoutoutText.addEventListener('mouseenter', function () {
        this.style.transform = 'scale(1.05)';
        this.style.textShadow = '0 0 50px rgba(212, 185, 150, 0.8)';
        this.style.borderColor = '#d4b996';
      });

      shoutoutText.addEventListener('mouseleave', function () {
    ...
    // Add hover effect to "Thanks" text
    const thanksText = document.querySelector('.line4');
    if (thanksText) {
      thanksText.addEventListener('mouseenter', function () {
        this.style.color = '#ffffff';
        this.style.textShadow = '0 0 20px rgba(212, 185, 150, 0.7)';
      });

      thanksText.addEventListener('mouseleave', function () {
        this.style.color = '#d4b996';
  ...
  function initialize() {
    ...
    setTimeout(() => {
      showNotification("Welcome to our Services Page! Scroll down for special thanks 🏆", 4000);
    }, 1500);
    ...
    window.addEventListener('scroll', function () {
      ...
      scrollTimeout = setTimeout(() => {
        if (window.scrollY > 300 && !vidTwoSection.classList.contains('visible')) {
          showNotification("↓ Scroll for special thanks message ↓", 2000);
        }
      }, 500);
    });
  }
